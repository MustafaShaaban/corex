...
| Team-safe company-site readiness (Spec 061) | **In progress.** PR A (this milestone's foundation): Role Gate (4 modes) + start prompts + `sites/<client>/` source layout + handoff format + make:site governance stubs + shared-host `dist` builder/verifier + Azure pipeline + deploy docs. Deferred task groups: PR B Media/WebP settings+CLI+delivery, PR C make:site `sites/` restructure + header/footer scaffolding + client image pipeline, PR D WP Font Library. Targets release v0.29.0 | High | v0.28.0; stable generator + deploy contracts |
