...
export default function MutationPreviewDialog( {
	preview,
	pending,
	close,
	apply,
	...
	const count =
		preview.record_ids?.length ||
		( preview.operation === 'create' ? 1 : 0 );
	return (
		<Modal
		...
		>
			...
			<p>
				{ sprintf(
					...
					mutationLabel( preview.operation ),
					count
				) }
			</p>
			{ Object.keys( preview.values || {} ).length > 0 && (
				<dl className="corex-data__preview-values">
