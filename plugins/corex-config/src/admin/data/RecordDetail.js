...
export default function RecordDetail( { explorer, record, close, edit } ) {
	...
	return <Modal title={ __( 'Record detail', 'corex' ) } onRequestClose={ close }>
		{ rows.length === 0
			? <p className="corex-data__empty">{ __( 'This record has no readable fields.', 'corex' ) }</p>
			: <dl className="corex-data__fields">{ rows.map( ( row ) => <div key={ row.key } className="corex-data__field">
