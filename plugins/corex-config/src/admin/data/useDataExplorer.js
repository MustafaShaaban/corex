...
export function useDataExplorer( config ) {
	...
	const initialSource = initialCatalog.find(
		...
		async ( method, url, payload ) => {
			...
			if ( ! result?.envelope?.ok ) {
				throw new Error(
					message(
						result,
						__( 'The request failed. Try again.', 'corex' )
					)
				);
			}
	...
	useEffect( () => {
		...
		if ( allowed ) {
			dispatch( { type: 'source', sourceKey: allowed.key } );
		}
	...
	const previewMutation = useCallback(
		async ( operation, recordIds, values = {} ) => {
	...
	const applyMutation = useCallback( async () => {
		...
		try {
			...
			dispatch( {
				type: 'success',
				message: __( 'The record changes were applied.', 'corex' ),
			} );
			reload();
		} catch ( error ) {
			dispatch( { type: 'error', message: error.message } );
