...
describe( 'Data Models tab permissions and deep-linking', () => {
	it( 'shows a data-only user records, and nothing that reshapes the models', () => {
		...
		expect( allowedTabs( { data: true, models: false }, CAPABLE ).map( ( t ) => t.key ) ).toEqual( [ 'records' ] );
	} );
	...
	it( 'shows a models-only user everything except records', () => {
		...
		expect( allowedTabs( { data: false, models: true }, CAPABLE ).map( ( t ) => t.key ) ).toEqual( [
	...
	it( 'opens the requested tab when it is permitted', () => {
		expect( resolveTab( 'migrations', BOTH, CAPABLE ) ).toBe( 'migrations' );
		expect( tabFromUrl( '/wp-admin/admin.php?page=corex-data-models&tab=records', BOTH, CAPABLE ) ).toBe( 'records' );
	} );
	...
	it( 'falls back to the first permitted tab rather than rendering an empty shell', () => {
		...
		expect( resolveTab( 'records', { data: false, models: true }, CAPABLE ) ).toBe( 'models' );
		expect( resolveTab( 'migrations', { data: true, models: false }, CAPABLE ) ).toBe( 'records' );
		expect( resolveTab( 'nonsense', BOTH, CAPABLE ) ).toBe( 'models' );
		expect( tabFromUrl( '/wp-admin/admin.php?page=corex-data-models', BOTH, CAPABLE ) ).toBe( 'models' );
	} );
...
describe( 'Data Models tabs are capability-aware, not just permission-aware', () => {
	...
	it( 'hides every capability tab when there are no sources at all', () => {
		expect( allowedTabs( BOTH, [] ).map( ( t ) => t.key ) ).toEqual( [ 'models', 'records' ] );
		expect( allowedTabs( BOTH ).map( ( t ) => t.key ) ).toEqual( [ 'models', 'records' ] );
	} );

	it( 'shows a capability tab as soon as one model declares it', () => {
		expect( allowedTabs( BOTH, [ ...READ_ONLY, ...CAPABLE ] ).map( ( t ) => t.key ) ).toEqual( [
	...
	it( 'refuses to deep-link to a tab nothing can satisfy', () => {
		...
		expect(
			tabFromUrl( '/wp-admin/admin.php?page=corex-data-models&tab=migrations', BOTH, READ_ONLY )
		).toBe( 'models' );
	} );
} );
...
describe( 'Model capability summary', () => {
	it( 'states an available capability in words rather than capability keys', () => {
		const summary = capabilitySummary( {
			capabilities: { read: true, create: true, import_dry_run: true, export_csv: true, migrations: true, rollback: true },
		} );

		expect( summary.map( ( entry ) => entry.key ) ).toEqual( [
			'read', 'write', 'import', 'export', 'migrations',
		] );
		expect( summary.every( ( entry ) => entry.available ) ).toBe( true );
		expect( summary.find( ( entry ) => entry.key === 'migrations' ).explanation ).toContain( 'roll back' );
	} );

	it( 'explains an unavailable capability without ever saying "adapter"', () => {
		const summary = capabilitySummary( { capabilities: { read: true, export_csv: true } } );
		...
		expect( summary.map( ( entry ) => entry.explanation ).join( ' ' ) ).not.toContain( 'adapter' );
	} );

	it( 'does not promise a rollback for a migration that has none', () => {
		const summary = capabilitySummary( { capabilities: { migrations: true, rollback: false } } );
		const migrations = summary.find( ( entry ) => entry.key === 'migrations' );
	...
	it( 'survives a source with no capabilities at all', () => {
		expect( capabilitySummary( undefined ).every( ( entry ) => entry.available === false ) ).toBe( true );
	} );
} );
