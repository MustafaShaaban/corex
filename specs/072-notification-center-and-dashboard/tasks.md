...
- [~] **T013** [US4] Implement producers: Submission (new/assigned + Phase A email-failure), Email Studio failure, Job failure/export-complete, Access request, Security lockout/hardening, Readiness blocker/cleared. Dedup keys per spec.
  ...
  - [ ] Remaining producers: submission **assigned**; Email Studio failure; **export-complete** (JobFinishedEvent state=completed + export kind); Security lockout / hardening; Readiness blocker/cleared. Each needs a signal from its subsystem (most emit no domain event yet) — one slice each.
