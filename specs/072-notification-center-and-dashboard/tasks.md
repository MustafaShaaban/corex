...
- [~] **T015** [US2] `apply_filters('corex_admin_header_actions','')` in `AdminPage::open()`; `NotificationBell` renders the count (99+, true count in label); shell CSS + a real drawer component (focus trap, Escape, focus return) added to `ApprovedComponentInventory` with `EXPECTED_COUNTS` bumped.
  ...
  - [ ] **Drawer finish** — global enqueue so the bell is interactive on *every* CoreX screen (needs the `build/admin/index.js` asset-handle consolidation across the 5 product screens, or a dedicated notification bundle entry, to avoid double-loading); `ApprovedComponentInventory` + `EXPECTED_COUNTS` declaration; Playwright e2e (focus-trap/Escape/return — jsdom can't verify, and this repo has no React-render Jest harness).
