...
const GLYPHS = {
	...
	captcha: [
		'M12 3 19 5.4 V11 c0 4.3-3 6.9-7 8.3 C8 17.9 5 15.3 5 11 V5.4 Z',
		'M8.7 11.6 l2.3 2.3 l4.3-4.6',
	],
	media: [
		'M4 5h16v14H4z',
		'M4 16l4.5-4.5 3 3 4-4 4.5 4.5',
		'M9 9.5a1.4 1.4 0 1 0-.001-.001z',
	],
...
function glyphGroup( key, size, frac, color, strokeW ) {
	...
	return (
		`<g transform="translate(${ n( off ) } ${ n( off ) }) scale(${ n(
			s
		) })">` +
		`<g fill="none" stroke="${ color }" stroke-width="${ n(
			strokeW / s
...
function coreGlyph( size, frac, color ) {
	...
	const pad = ( u - d ) / 2;
	const at = ( cx, cy ) =>
		`<rect x="${ n( off + cx * u + pad ) }" y="${ n(
			off + cy * u + pad
		) }" width="${ n( d ) }" height="${ n( d ) }" rx="${ n(
			d * 0.26
		) }" fill="${ color }"/>`;
...
function cornerSigil( size, color ) {
	...
	const y = size * 0.64;
	const at = ( cx, cy ) =>
		`<rect x="${ n( x + cx * g ) }" y="${ n( y + cy * g ) }" width="${ n(
			dot
		) }" height="${ n( dot ) }" rx="${ n(
			dot * 0.28
		) }" fill="${ color }"/>`;
	return `<g opacity="0.95">${
		at( 0, 0 ) + at( 2, 0 ) + at( 1, 1 ) + at( 0, 2 ) + at( 2, 2 )
	}</g>`;
}
...
function tile(
	key,
	size,
	{ glyphColor = BRASS, sigilColor, proTick = false, label = '' } = {}
) {
	...
	const parts = [
		`<rect x="0.6" y="0.6" width="${ n( size - 1.2 ) }" height="${ n(
			size - 1.2
		) }" rx="${ n(
			r
		) }" fill="${ TILE }" stroke="${ TILE_BD }" stroke-width="1"/>`,
		isCore
			? coreGlyph( size, frac, glyphColor )
			: glyphGroup( key, size, frac, glyphColor, stroke ),
	];
	if ( showSigil && ! isCore ) {
		parts.push( cornerSigil( size, sigilColor ) );
	}
	if ( proTick ) {
		parts.push(
			`<circle cx="${ n( size * 0.78 ) }" cy="${ n(
				size * 0.22
			) }" r="${ n( size * 0.12 ) }" fill="${ BRASS }"/>`
		);
	}
...
function mark( size = 48 ) {
	const cell = ( x, y, fill ) =>
		`<rect x="${ x }" y="${ y }" width="12" height="12" rx="2.5" fill="${ fill }"/>`;
	return (
		`<svg xmlns="http://www.w3.org/2000/svg" width="${ size }" height="${ size }" viewBox="0 0 48 48" fill="none" role="img" aria-label="Corex">` +
		`<title>Corex</title>` +
		cell( 3, 3, 'currentColor' ) +
		cell( 33, 3, 'currentColor' ) +
		cell( 18, 18, BRASS ) +
		cell( 3, 33, 'currentColor' ) +
		cell( 33, 33, 'currentColor' ) +
		`</svg>\n`
	);
}
...
for ( const [ slug, key ] of Object.entries( MAP ) ) {
	...
	writeFileSync(
		`${ OUT }/${ slug }.svg`,
		tile( key, 48, { label: slug, proTick: false } )
	);
	writeFileSync(
		`${ OUT }/${ slug }--disabled.svg`,
		tile( key, 48, {
			glyphColor: MUTED,
			sigilColor: MUTED,
			label: `${ slug } (disabled)`,
...
writeFileSync(
	`${ OUT }/pro.svg`,
	tile( 'pro', 48, { proTick: true, label: 'pro' } )
);
writeFileSync(
	`${ OUT }/pro--disabled.svg`,
	tile( 'pro', 48, {
		glyphColor: MUTED,
		sigilColor: MUTED,
		proTick: true,
...
writeFileSync(
	`${ OUT }/fallback.svg`,
	tile( 'core', 48, { label: 'add-on' } )
);
writeFileSync(
	`${ OUT }/fallback--disabled.svg`,
	tile( 'core', 48, { glyphColor: MUTED, label: 'add-on (disabled)' } )
);
...
console.log(
	`Generated ${ count } add-on logo SVGs in ${ OUT } + corex-mark.svg`
);
