...
const isForbidden = ( absPath ) => {
	...
	return FORBIDDEN_SEGMENTS.some( ( seg ) =>
		parts.includes( seg.toLowerCase() )
	);
};
...
export function buildPlan( cfg ) {
	...
	if ( existsSync( p( 'wp' ) ) ) {
		for ( const entry of readdirSync( p( 'wp' ) ) ) {
			...
			copies.push( {
				from: p( 'wp', entry ),
				to: join( distDir, entry ),
				kind: 'core',
			} );
		}
	} else {
		warnings.push(
			'wp/ not found — the artifact will not include WordPress core; supply core on the target.'
		);
	}
	...
	for ( const dir of [ 'plugins', 'addons' ] ) {
		...
		for ( const entry of readdirSync( p( dir ) ) ) {
			...
			copies.push( {
				from: src,
				to: join( distDir, 'wp-content', 'plugins', entry ),
				kind: 'plugin',
			} );
	...
	if ( existsSync( p( 'theme' ) ) ) {
		copies.push( {
			from: p( 'theme' ),
			to: join( distDir, 'wp-content', 'themes', 'corex' ),
			kind: 'theme',
		} );
	...
	if ( client ) {
		...
		if ( ! existsSync( siteRoot ) ) {
			warnings.push(
				`sites/${ client }/ not found — no client source packaged.`
			);
		} else {
			...
			for ( const entry of readdirSync( siteRoot ) ) {
				...
				if ( entry === 'plugins' ) {
					for ( const pl of readdirSync( src ) ) {
						copies.push( {
							from: join( src, pl ),
							to: join( distDir, 'wp-content', 'plugins', pl ),
							kind: 'client-plugin',
						} );
						pluginNames.push( pl );
					}
					...
					for ( const th of readdirSync( src ) ) {
						copies.push( {
							from: join( src, th ),
							to: join( distDir, 'wp-content', 'themes', th ),
							kind: 'client-theme',
						} );
						themeNames.push( th );
					}
					...
					copies.push( {
						from: src,
						to: join( distDir, 'wp-content', 'themes', entry ),
						kind: 'client-theme',
					} );
...
export function verifyClientAssets( distDir ) {
	...
	for ( const theme of readdirSync( themesDir ) ) {
		...
		const checks = [
			{
				src: join( 'assets', 'src', 'scss' ),
				out: join( 'assets', 'css' ),
				ext: '.css',
				kind: 'SCSS',
			},
			{
				src: join( 'assets', 'src', 'js' ),
				out: join( 'assets', 'js' ),
				ext: '.js',
				kind: 'JS',
			},
		...
		for ( const { src, out, ext, kind } of checks ) {
			if ( ! existsSync( join( themeDir, src ) ) ) {
				continue;
			}
			const outDir = join( themeDir, out );
			const built =
				existsSync( outDir ) &&
				readdirSync( outDir ).some( ( f ) =>
					f.toLowerCase().endsWith( ext )
				);
			if ( ! built ) {
				errors.push(
					`client theme "${ theme }": ${ kind } source present but no compiled ${ out }/*${ ext } (run the theme's npm run build before packaging)`
				);
			}
		}
	}
...
const isMain =
	basename( process.argv[ 1 ] ?? '' ) === 'build-shared-host-dist.mjs';
if ( isMain ) {
	...
	console.log(
		`${ dryRun ? '[dry-run] ' : '' }packaging ${
			...
			( client
				? ` (client: ${ client })`
				: ' (framework only — pass --client=<slug> to include a client site)' )
	);
	for ( const op of plan.copies ) {
		console.log(
			`  ${ op.kind.padEnd( 14 ) } ${ relative(
				repoRoot,
				op.from
